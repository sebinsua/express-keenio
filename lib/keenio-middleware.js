...
KeenioMiddleware.prototype.isAcceptableStatusCode = function (statusCode) {
  ...
  var firstCharacter = String(statusCode).charAt(0);
...
KeenioMiddleware.prototype._finishResponse = function (req, _parsedResponseData, routeConfig) {
  ...
  return function onResponseFinish() {
    ...
    var defaultWhitelistProperties = { whitelistProperties: self.options.whitelistProperties };
    ...
    if (self.options.hasOwnProperty('routes')) {
      ...
      // which should override the `defaultWhitelistProperties` which are set earlier.
      // (Additionally the `eventCollectionName` and `tag` are also overridden in this order.)
      routeConfig = helpers.extend({}, defaultWhitelistProperties, eventCollectionMetadata, routeConfig);
    } else {
      routeConfig = helpers.extend(defaultWhitelistProperties, routeConfig);
    }
