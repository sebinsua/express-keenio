...
KeenioMiddleware.prototype._finishResponse = function (req, _parsedResponseData, routeConfig) {
  ...
  return function onResponseFinish() {
    ...
    var isWhitelistExplicitlyDefined = self.options.whitelistProperties || routeConfig.whitelistProperties;
    
    var _configs = [];

    // If a route has data stored in the 'routes' key of the configuration, we fetch this data.
    var _hasRouteConfig = self.options.hasOwnProperty('routes');
    if (_hasRouteConfig) {
    ...
    var _optionsWhitelist = self.options.whitelistProperties,
        _preConfig;
    if (isWhitelistExplicitlyDefined) {
      _preConfig = [{}, _optionsWhitelist];
      _configs = _preConfig.concat.apply(_preConfig, _configs);
    } else {
      var _whitelistPropertiesConfig = { whitelistProperties: self.routeSchemas.getWhitelist(req.route) };
      _preConfig = [{}, _whitelistPropertiesConfig, _optionsWhitelist];
      _configs = _preConfig.concat.apply(_preConfig, _configs);
    }
    routeConfig = helpers.extend.apply(helpers, _configs);
