...
KeenioMiddleware.prototype._generateHandler = function (props) {
  ...
  return function keenioHandler(req, res, next) {
    ...
    res.on("finish", function () {
      ...
      if (!req.route) { // If a normal middleware had been run, without a route this would be the case. E.g. favicon.ico
        return false;
      }

      if (handleMiddlewareScope._isExcludedRoute(req.route)) {
        return false;
      }

      var eventCollection, keenEvent;

      var eventCollectionMetadata = handleMiddlewareScope._getRouteConfig(req.route);
      if (eventCollectionMetadata === false) { // @todo: Why??!!!
        return false;
      }

      props = helpers.extend(eventCollectionMetadata, props);

      eventCollection = props.eventCollectionName ? props.eventCollectionName : handleMiddlewareScope.generateEventCollectionName(req.route);
      keenEvent = handleMiddlewareScope.generateKeenEvent(req, { reaction: _reaction, status: _status }, props);
...
KeenioMiddleware.prototype.generateKeenEvent = function (req, parsedResponseData, props) {
