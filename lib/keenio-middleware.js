...
KeenioMiddleware.prototype._finishResponse = function (req, _parsedResponseData, routeConfig) {
  ...
  return function onResponseFinish() {
    ...
    var isWhitelistExplicitlyDefined, config;
    try {
...
KeenioMiddleware.prototype._overrideConfig = function (route, routeConfig) {
  ...
  var isWhitelistExplicitlyDefined = this.options.whitelistProperties || routeConfig.whitelistProperties;
  ...
  if (_hasRouteConfig) {
    ...
    if (!eventCollectionMetadata) { throw new Error("Config has a routes key, but this route is not defined."); }
