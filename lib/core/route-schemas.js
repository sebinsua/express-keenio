...
RouteSchemas.prototype.getWhitelist = function (route) {
  ...
  if (this.routes[routeIdentifier]) {
    var route = this.routes[routeIdentifier],
        isCrystallised = !!route.cystallised;
    
    if (!isCrystallised) {
      for (var i = 0; i < schemas.length; i++) {
        if (route[schemas[i]].frozen) {
          var keyWhitelist = this._generateWhitelist(route[schemas[i]], schemas[i]);
      ...
      whitelist = route.crystallised;
    }
  }

  if (allFrozen) {
    route.crystallised = whitelist;
...
RouteSchemas.prototype.load = function () {
  ...
  if (fs.existsSync(cachePath)) {
    ...
    whitelists.forEach(function (whitelist, routeIdentifier) {
