...
function RouteSchemas(options) {
  ...
  // @todo: currently the cache is not being set and this is causign the proble,
  // @todo: additionally i need to test that load is working.

  this.cache = this.options.defaults && this.options.defaults.eventualSchemas && this.options.defaults.eventualSchemas.cache || true;
...
RouteSchemas.prototype.save = function (route, whitelist) {
  ...
  fs.readFile(cachePath, function (err, originalData) {
    ...
    fs.writeFile(cachePath, JSON.stringify(whitelists), function () { });
  });
