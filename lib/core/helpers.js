...
var setDefaultEvents = function (eventEmitter, events) {
  ...
  for (var ie = 0; ie < events.length; ie++) {
    ...
    if (eventEmitter.listeners(eventName).length === 0) {
      fn = noop;
      if (eventName === 'error') {
        fn = function defaultErrorHandler(err) {
          var errMessage = [
            err.code,
            err.stack
          ].join(' ');
          ...
          if (err.collection && err.event) {
            var extendedErrMessage = [
              err.code,
              "Collection: " + err.collection,
              "Event: " + JSON.stringify(err.event, null, 2)
            ].join(' ');
            console.error(extendedErrMessage);
          }
        };
      }
